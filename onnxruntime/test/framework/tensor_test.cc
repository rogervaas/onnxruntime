...
TEST(TensorTest, SizeOverflow) {
  // shape overflow
  ASSERT_THROW(TensorShape({std::numeric_limits<int64_t>::max() / 2, 3}).Size(), OnnxRuntimeException);

  auto type = DataTypeImpl::GetType<float>();
  auto alloc = TestCPUExecutionProvider()->GetAllocator(0, OrtMemTypeDefault)->Info();

  // total size overflow with 4 bytes per element
  TensorShape shape1({static_cast<int64_t>(std::numeric_limits<size_t>::max() / 2)});
  ASSERT_THROW(Tensor(type, shape1, nullptr, alloc), OnnxRuntimeException);
  ...
  ptrdiff_t offset = 1;
  ASSERT_THROW(Tensor(type, shape2, nullptr, alloc, offset), OnnxRuntimeException);
}
}  // namespace test
}  // namespace onnxruntime
